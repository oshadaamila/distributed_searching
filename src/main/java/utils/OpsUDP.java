...
public class OpsUDP {
    ...
    public void processMessage(String msg, InetAddress incomingIP, int port) throws IOException {
        ...
        synchronized (node.getNeighbours()) {
            if (command.equals("JOIN")) {
                ...
                this.node.incrementLeaveCount();
                if(this.node.getNeighbours().size() == this.node.getLeaveCount()){
                    System.exit(0);
                }
            }
        }
    ...
    private void processUNROK(StringTokenizer st) {
        ...
        if (value.equals("0")) {
            this.node.setUnregistered(true);
            System.out.println("unregistered successfully");
        } else {
            this.node.setUnregistered(false);
        ...
        try {
            ...
            if (response != null) {
                ...
                if (st.nextToken().equals("LEAVEOK")) {
                    ...
                    if(this.node.getNeighbours().size()==1){
